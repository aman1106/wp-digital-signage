...
jQuery(document).ready(function () {
    ...
    jQuery('#always_time').change(function () {
        if (jQuery('#always_time').is(':checked')) {
            jQuery("#dpd1").prop('disabled', true);
            jQuery("#dpd2").prop('disabled', true);
            jQuery("#dpd1").prop('value', "");
            jQuery("#dpd2").prop('value', "");
        }
        else {
            jQuery("#dpd1").prop('disabled', false);
            jQuery("#dpd2").prop('disabled', false);
        }
    });
...
if(jQuery('#edit_flag').val()=='1'){
    ...
    var val = jQuery('.fm_add').val();
        if (val == '0') {
            ...
            //var marker = L.marker([ height / scale / 2, width / scale / 2], {icon: blue}).addTo(map);
            ...
            map.on('click', function (event) {
               // alert(event.latlng.lat);
                ...
                marker.setLatLng([Math.round(event.latlng.lat), Math.round(event.latlng.lng)]);
                jQuery("#lat").val(Math.round(event.latlng.lat));
                jQuery("#lng").val(Math.round(event.latlng.lng));
                jQuery('#AddSensor').show();
            });
            jQuery(".leaflet-image-layer").attr('src', site_url + '/wp-content/uploads/fm-' + val);
            //jQuery(".fm_img").attr('src', site_url + '/wp-content/uploads/fm-' + val);
            jQuery("#floormap").show();
        }
}

    jQuery('.fm_add').change(function () {
