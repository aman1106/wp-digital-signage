              ...
              $delete = $wpdb->delete("wpds_events", array('id' => $event));
                if (!$delete) {
                    ...
                    <div class="notice notice-error is-dismissible">
                        <p><?php _e('Could not delete Event. Please try again or contact support', 'sample-text-domain'); ?></p>
                    </div>
                    <?php
                } else if ($delete > 0) {
                    ?>
                    <div class="notice notice-success is-dismissible">
                        <p><?php _e('Successfully deleted ' . $delete . ' Events', 'sample-text-domain'); ?></p>
                    </div>
                    <?php
                }
            }
...
//$this->items = $a;
    ...
    function column_id($item) {
        $actions = array(
            'edit' => sprintf('<a href="?page=wpds_add_event&edit_event=%s">Edit</a>', $item['id']),
            'delete' => sprintf('<a href="?page=page=wpds_events&del_event=%s">Delete</a>', $item['id']),
        );
...
echo '<div class="wrap"><h2> Events Manager<a style="float:right" href="admin.php?page=wpds_add_event"><input type="submit" name="show_add" value="Create New Event »" class="button-primary"></a></h2><br />';?>
